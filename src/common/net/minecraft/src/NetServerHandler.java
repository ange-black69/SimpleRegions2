...
public class NetServerHandler extends NetHandler
{
    ...
    public void handleBlockDig(Packet14BlockDig par1Packet14BlockDig)
    {
        ...
        else
        {
            ...
            if (par1Packet14BlockDig.status == 0)
            {
            	if(!var3 && dries007.SimpleRegions.regions.VanillaInterface.hasTag(var2, var5, var6, var7, "noplayerblock" , getPlayer()))
            	{
    ...
    public void handlePlace(Packet15Place par1Packet15Place)
    {
        ...
        {
        	if(dries007.SimpleRegions.regions.VanillaInterface.hasTag(var2, "noplayeritem", getPlayer(), true))
            {
            	return;
            }
            if (var3 == null)
        ...
        else
        {
            ...
            double dist = playerEntity.theItemInWorldManager.getBlockReachDistance() + 1;
            dist *= dist;
            if (this.hasMoved && this.playerEntity.getDistanceSq((double)var5 + 0.5D, (double)var6 + 0.5D, (double)var7 + 0.5D) < dist && (var12 > mcServer.spawnProtectionSize || var9) && (var9 || !dries007.SimpleRegions.regions.VanillaInterface.hasTag(var2, var5, var6, var7, "noplayerblock", getPlayer())))
            {
